...
var _ = Describe("gogen", func() {
	...
	It("runs and has output for Sacramento", func() {
		...
		Expect(sessionString).To(ContainSubstring("Found 12 convictions that are eligible for dismissal"))
		...
		Expect(sessionString).To(ContainSubstring("Found 2 convictions that are eligible for reduction"))
		Expect(sessionString).To(ContainSubstring("Found 1 11358 convictions that are eligible for reduction"))
		...
		Expect(sessionString).To(ContainSubstring("Found 8 convictions in this county with eligibility reason: Conviction older than 10 years"))
